...
def generate_agent_summary():
    ...
    try:
        ...
        # --- OUTPUT ---
        original_sheets['Agentes'] = final_df
        
        # Obtener Provider del DF
        provider = "UNKNOWN"
        # Usamos df original (Detalle_Leads_Unicos) que tiene 'provider'
        if 'provider' in df.columns:
             uniq = df['provider'].dropna().unique()
             if len(uniq) > 0:
                 raw_p = str(uniq[0]).upper().strip()
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
             ...
             wb = writer.book
             if 'Agentes' in wb.sheetnames:
                 ws = wb['Agentes']
                 apply_comments(ws, final_df.columns, "Agentes")
