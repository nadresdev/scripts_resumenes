...
COMMENTS_EJECUTIVO = {
    "Total leads recibidos": "Total de registros recibidos del proveedor",
    "Leads Unicos analizados": "Registros únicos en el periodo",
    "Contactados (unicos)": "Leads con al menos 1 TMO > 0",
    "No contactados (Unicos)": "Leads con TMO siempre 0 o nulo",
    "Contactabilidad % (unicos)": "contactados / leads_unicos * 100",
    "Leads Cerrados": "Leads con status contiene CERRADO",
    "Ventas (leads insertados mes)": "Leads con lastOcmCoding = VENTA/POLIZA",
    "interacciones_contacto": "Suma de interacciones con TMO > 0",
    "interacciones_sin_contacto": "Suma de interacciones con TMO = 0 (intentos)",
    "Conversin % (contactados)": "ventas / contactados * 100",
    "conversion sobre interacciones con contacto_%": "interacciones_venta / interacciones_contacto * 100",
    "conv_contactado_cerrado_%": "ventas (cerradas) / contactados * 100",
    "interacciones en venta": "Interacciones resultDesc = VENTA/POLIZA",
    "Total interacciones": "Total de intentos (contacto + sin contacto)",
    "TMO totalizado (hh:mm:ss)": "Suma total de TMOs > 0",
    "TMO mediana (General) (hh:mm:ss)": "Mediana de TMOs > 0 (Todos)",
    "TMO interacciones (venta) (hh:mm:ss)": "Suma TMOs donde resultDesc = VENTA",
    "TMO mediana (venta) (hh:mm:ss)": "Mediana TMOs donde resultDesc = VENTA",
    "TMO mediana (no venta) (hh:mm:ss)": "Mediana TMOs donde resultDesc != VENTA", 
    "SLA OPERATIVO (10-20 Lu-Vi)": "Mediana SLA (L-V 10-20h)",
    "SLA EXTRAHORARIO (Lu-Vi)": "Mediana SLA (L-V fuera de 10-20h)",
    "SLA FIN DE SEMANA": "Mediana SLA (Sábados y Domingos)",
    "ACW Mediana (hh:mm:ss)": "Mediana del tiempo ACW por lead",
    "Total tiempo llamadas": "Suma total de timeCall (segundos)"
}
...
def apply_styles_and_order():
    ...
    try:
        ...
        # --- 4. LIMPIEZA DE CARPETAS INTERMEDIAS ---
        # clean_intermediate_directories(kpi_smart_dir)
...
def clean_intermediate_directories(base_dir):
    """
    Limpia el contenido de las carpetas intermedias generadas durante el proceso,
    respetando ORIGEN y ENTREGABLES.
    """
    print("\n--- INICIANDO LIMPIEZA DE CARPETAS INTERMEDIAS ---")
    
    # Lista explicita de carpetas a limpiar (Excluye ORIGEN, ENTREGABLES, PROCESSED_BATCH)
    folders_to_clean = [
        "LEADS_UNICOS",
        "DETALLE_LEADS_UNICOS",
        "RESUMEN_DIARIO",
        "RESUMEN_AGENTES", # Windows es case-insensitive pero mantenemos nombre usado
        "RESUMEN_SEMANAL",
        "RESUMEN_EJECUTIVO",
        "FRECUENCIA"
    ]
    ...
    for folder in folders_to_clean:
        ...
        if os.path.exists(folder_path):
            ...
            for f in files:
                try:
                    # No borrar subcarpetas si las hubiera, solo archivos
                    if os.path.isfile(f):
                        os.remove(f)
             ...
             print(f"Carpeta no encontrada (saltando): {folder}")
